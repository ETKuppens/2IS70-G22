...
public class MapActivity extends CollectorBaseActivity implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    public List<LatLng> geoMarkersLocations = new ArrayList<>();
    ...
    void nearestDistance() {
        ...
        int minIndex = -1;
        float[] resultedDistance = new float[1];
        for (int i = 0; i < geoMarkersLocations.size(); i++) {
            Location.distanceBetween(geoMarkersLocations.get(i).latitude,
                    geoMarkersLocations.get(i).longitude,
                    currentLocation.latitude, currentLocation.longitude, resultedDistance);
            double strainghtDistance = resultedDistance[0];
            if (MinDistance < strainghtDistance) {
                MinDistance = strainghtDistance;
                minIndex = i;
            }
            MinDistance = Math.min(MinDistance, strainghtDistance);
        }
        Context context = getApplicationContext();
        CharSequence text = "nearest pin: " + Double.toString(MinDistance);// + " min index: " +
    ...
    public void onMapReady(GoogleMap map) {
        ...
        /*Location geoMarkerLocation = new Location("");
        geoMarkerLocation.setLatitude(geoMarkerLocationLatLng.latitude);
        geoMarkerLocation.setLongitude(geoMarkerLocationLatLng.longitude);
